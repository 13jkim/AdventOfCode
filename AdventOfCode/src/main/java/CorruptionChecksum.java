...
//        String[][] stringCorruptionArr = new String[inputArr.length][];
//        for (int i = 0; i < stringCorruptionArr.length; i++) {
//            stringCorruptionArr[i] = inputArr[i].split("\t");
//        }
//        int[][] corruptionArr = new int[stringCorruptionArr.length][];
//
//        for (int i = 0; i < corruptionArr.length; i++) {
//            corruptionArr[i] = new int[stringCorruptionArr[i].length];
//        }
//
//        for (int i = 0; i < corruptionArr.length; i++) {
//            for (int j = 0; j <corruptionArr[i].length; j++) {
//                corruptionArr[i][j] = Integer.valueOf(stringCorruptionArr[i][j]);
//            }
//        }
        //Stream string to int[][]
        int[][] iArr = Arrays.stream(input.split("\n")).map(r
                        ->
                        Arrays.stream(r.split("\t")).mapToInt(Integer::parseInt).toArray()
        ).toArray(int[][]::new);

        System.out.println(checkSum(iArr));
    }

    public static int checkSum(int[][] corruptionArr) {
        int sum = 0;

        for (int i = 0; i < corruptionArr.length; i++) {
            Arrays.sort(corruptionArr[i]);
            sum += corruptionArr[i][corruptionArr[i].length - 1] - corruptionArr[i][0];
        }
